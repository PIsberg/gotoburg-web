...
- `node scripts/make-brand-assets.mjs` — regenerate `public/logo.png` and the two favicon PNGs. Maintenance tool, not part of the build. Chromium rasterises the wordmark because Playwright is already a dependency and nothing else in the toolchain turns text into a PNG; the type stack and colour mirror `components/Layout.tsx`. `logo.png` is the schema.org `Organization` logo referenced on every page, and it 404'd from the day the JSON-LD was written until 2026-08-19, which silently invalidates the logo property.
...
- `Article` shape is in the root `types.ts`. `content` is `string[]` — one entry per paragraph — specifically so ad slots can be inserted between paragraphs. An entry beginning `## ` renders as an `<h2>` instead (see `Block` in `pages/ArticlePage.tsx`). The one inline form that is interpreted is `[etikett](href)`, where `href` is an `https://` URL or a site-relative path; it renders as an anchor (external ones get `target="_blank"` and `rel="noopener noreferrer"`, internal ones go through react-router). Nothing else is: `**bold**` and `*   ` list markers ship as literal asterisks, which is what the pre-2026-08 articles did. `src/seo.ts` strips the link syntax before counting words for schema.org `wordCount`.
...
- A `PageMeta` with `indexable: false` is still prerendered and still linked, but gets `<meta name="robots" content="noindex, follow">` and is left out of `sitemap.xml`. The two decisions are made in one place because a noindex URL inside a sitemap is an error Search Console reports. Category pages set it from `MIN_ARTICLES_TO_INDEX_CATEGORY` (3): on 2026-08-28 `/kategori/kultur` and `/kategori/arbete` listed one article each and rendered 152 words of visible text, mostly nav and footer, and only 20 of the 36 submitted URLs were articles. Write the articles and the page re-enters the sitemap on the next build; the count is printed by `npm run build`.
...
- `tests/e2e/renders.spec.ts` is the counterpart, and exists because that blind spot was real: `/explore` served a blank white page in a browser for months while every SEO assertion passed, since the prerendered HTML was never the problem. It drives a real browser, asserts every route paints and throws nothing, and pins the ad-slot, dead-control and noindex rules. `playwright.config.ts` pins an invalid `VITE_GOOGLE_MAPS_API_KEY` for the build so `/explore` always exercises the load-the-Maps-API path; without it CI has no key, the component short-circuits, and the test would assert the page survives a map it never loaded.
...
- `sitemap.xml` is submitted and read **Success, 35 discovered pages** on 2026-08-19, matching the route count the build printed then. The build prints that count on every run and it grows with the article set (36 as of 2026-08-23). It is regenerated on every build, so it never needs re-submitting after a content change; Google re-fetches it.
...
- **Google Maps** (`components/GoogleMapSection.tsx`, used by ExplorePage): requires `VITE_GOOGLE_MAPS_API_KEY` in `.env.local` locally, and as a Netlify env var in production. Uses the **Maps JavaScript API only** — no Geocoding, despite what this file used to say, and `libraries=places` was loaded without a single Places call until 2026-08-19. Articles can carry an optional `googleMapsUrl`; `placeCoordinates()` parses `@lat,lng` out of it and is the single source for both the markers and the list on `/explore`.
- **The map does not work in production and has not for some time.** The Cloud project behind the key has no billing enabled, so the API answers `BillingNotEnabledMapError`. Verified 2026-08-19 in the browser console, and again 2026-08-28 with a Static Maps probe that returned `403 You must enable Billing`. `/explore` therefore renders its places as real markup and treats the map as an enhancement; do not move that content back inside the map.
- **`loading=async` means `script.onload` fires before `google.maps.Map` exists.** Constructing a Map on `onload` throws `google.maps.Map is not a constructor` out of an effect, React unmounts the whole tree, and the route serves a blank white page. That is what production did on `/explore`, the first link in the main nav, from the day the loader gained `loading=async` until 2026-08-28. `useGoogleMapsScript` now awaits `google.maps.importLibrary('maps')` before reporting loaded, and both map effects are wrapped so nothing the Maps API throws can escape. Do not go back to setting loaded on `onload`.
- `components/ErrorBoundary.tsx` wraps the route tree in `App.tsx` so the next uncaught render error costs a degraded panel rather than the entire site. It is a backstop, not a licence to let effects throw.
- The Maps key is a **browser** key. It ships in `dist/assets/index-*.js` and is readable by anyone who views source, by design — `git` history is not what exposes it, and scrubbing history would not make it private. The control is restriction in the Cloud console (HTTP referrers plus API restrictions), not secrecy. GitHub secret-scanning alert #1 flags it for this reason.
...
- **AdSense**: publisher ID `ca-pub-2203695397498260` is wired into `index.html` and `src/constants.ts` (`ADSENSE_CONFIG`). Ad slots are inserted via `components/AdSense.tsx` at: header (`Layout.tsx`), home feed middle + sidebar (`HomePage.tsx`), in-article + sidebar (`ArticlePage.tsx`).
- **The five slot ids in `ADSENSE_CONFIG` are real ad units**, created in the console on 2026-08-28. They were placeholders (`header-banner-12345`, `feed-middle-56789`, ...) until then, and AdSense reserves layout height for an `<ins>` it cannot fill, so every page opened with roughly 330px of blank white between the nav and the first line of content. `AdSense.tsx` still renders nothing at all for a slot that fails `isServableSlot` (numeric, six digits or more), so a regression to placeholder ids costs an empty gap rather than a reserved one. `adsbygoogle.js` stays in `index.html` regardless, because AdSense needs it to verify the site.
- **Ad slots are held off the page until AdSense approves the site.** `ADSENSE_ENABLED` in `src/constants.ts` is false unless `VITE_ADSENSE_ENABLED=true` is set in the build, and `AdSense.tsx` renders nothing while it is false. This is not caution: measured on production 2026-08-28 with the real unit ids live, `adsbygoogle.js` marked every `<ins>` `data-adsbygoogle-status="done"` and set an inline `height: 280px`, but no ad comes back for an unapproved site, so each slot was a reserved empty box and every page opened with 312px of blank white above the first line of content. Collapsing on `ins[data-ad-status="unfilled"]` cannot help, because that attribute is only set once a fill is attempted and before approval it is never set at all. Flip the env var on approval day; nothing else changes. `adsbygoogle.js` stays in `index.html` throughout, since that is the site-level code AdSense verifies against.
- `tests/e2e/renders.spec.ts` branches on `process.env.VITE_ADSENSE_ENABLED`, not on the imported `ADSENSE_ENABLED`: that constant reads `import.meta.env`, which Vite fills at build time and which is undefined in the Node process the tests run in, so importing it would make the enabled branch silently never run.
- **Three of the units are display and two are native, and the native ones are not interchangeable with them.** `gotoburg-in-article` needs `data-ad-layout="in-article"`; `gotoburg-home-feed-middle` needs `data-ad-layout-key`, a value generated with the unit that encodes the layout picked when it was created and cannot be guessed or shared. Both are passed through `AdSense.tsx`'s `layout` and `layoutKey` props and the key lives in `ADSENSE_CONFIG` beside its slot id. Omitting either leaves a unit that still serves, but as a plain display ad rather than the native format, which no id check would catch: `tests/e2e/renders.spec.ts` asserts the attributes directly. The README's "replace ca-pub-XXXX" instructions are stale — the real publisher ID is already in place.
