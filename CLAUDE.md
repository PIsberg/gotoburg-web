...
- **Most lead images are not photographs of their subject.** No free photograph exists of a small restaurant, so the manifest points at the street it stands on. Uncaptioned, with the headline as alt text, that asserts the picture shows the place, which the site cannot stand behind and which reads to a reviewer as never having visited. `article.imageCaption` names what is actually in the frame and is rendered above the attribution; `pages/ArticlePage.tsx` uses it as the alt text so a screen reader is not told the opposite. **Absent means the photograph is of the subject**, so do not add a caption that restates the headline: the empty case carries meaning. `tests/e2e/seo.spec.ts` asserts the caption is served and that the alt text agrees with it.
- This is a mitigation, not the fix. The fix is photographing the places, which is issue #21.
