...
- **The five slot ids in `ADSENSE_CONFIG` are placeholders** (`header-banner-12345`, `feed-middle-56789`, ...), not ad units. A real unit id is numeric. AdSense reserves layout height for an `<ins>` it cannot fill, so until 2026-08-28 every page opened with roughly 330px of blank white between the nav and the first line of content, measured in the browser. `AdSense.tsx` now renders nothing at all for a slot that fails `isServableSlot`; replacing the ids with the real ones from the AdSense console lights every slot up with no other change. `adsbygoogle.js` stays in `index.html` regardless, because AdSense needs it to verify the site. The README's "replace ca-pub-XXXX" instructions are stale — the real publisher ID is already in place.
